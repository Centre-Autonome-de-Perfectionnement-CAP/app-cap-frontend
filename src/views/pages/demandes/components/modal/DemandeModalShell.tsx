...
const buildFileList = (demande: DocumentRequest): FileItem[] => {
  ...
  const push = (arr: any[], cat: FileItem['category']) => {
  ...
  push(demande.files,           'soumission')
  push(demande.complement_files,'complement')
  push(demande.secretary_files, 'secretaire')
  return items
}
...
const FullViewer = ({ initialFile, allFiles, onClose }: FullViewerProps) => {
  ...
  const handleSelectFile = (file: FileItem) => {
    ...
    setState(prev => {
      ...
      if (!prev.right) return { ...prev, right: file, right: file, active: prev.active }
