...
import { cilCheckAlt, cilX, cilWarning, cilReload } from '@coreui/icons'
...
const DetailModal = ({ demande, visible, onClose, onAction }: {
  ...
  const footer = (<>
    ...
    {inCircuit ? (
      // ── En circuit de correction : seule action autorisée ──────────────────
      <ActionButton
        label="Renvoyer à la Secrétaire"
        icon={cilReload}
        color="warning"
        loading={loading}
        onClick={() => run('return_to_secretaire')}
      />
    ) : (
      // ── Circuit normal ─────────────────────────────────────────────────────
      <>
      ...
      <DemandeDetailBase demande={demande}>
        {inCircuit ? (
          <CAlert color="warning" className="mt-3 py-2 small">
            ...
            cliquez sur <strong>"Renvoyer à la Secrétaire"</strong> pour qu'elle
