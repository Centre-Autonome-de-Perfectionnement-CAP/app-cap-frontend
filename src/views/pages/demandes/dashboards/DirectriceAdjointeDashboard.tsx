...
import { cilX, cilCheck, cilReload } from '@coreui/icons'
    ...
    <DemandeModalShell demande={demande} visible={visible} onClose={onClose}
      ...
      <DemandeDetailBase demande={demande}>
        {inCircuit ? (
          ...
          <>
            <CAlert color="warning" className="mt-3 py-2 small">
              <strong>Attention :</strong> En signant, le dossier passe à la Sec. Directeur. Action définitive.
            </CAlert>
...
const DirectriceAdjointeDashboard = () => {
  ...
  return (
    <DirectionShell title="Gestion des demandes" subtitle="Directrice Adjointe" hideHeader>
      <CCard className="border-0" style={{ boxShadow: '0 2px 16px rgba(0,0,0,0.07)', borderRadius: 14 }}>
        <CCardHeader className="bg-white" style={{ borderBottom: '1px solid #f1f5f9', borderRadius: '14px 14px 0 0', padding: '16px 20px 12px' }}>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
              <div style={{ fontWeight: 700, fontSize: '1.05rem', color: '#111827' }}>Documents à signer</div>
      ...
      {selected && <DetailModal demande={selected} visible={detailOpen} onClose={closeDetail} onAction={handleAction} />}
    </DirectionShell>
  )
}
