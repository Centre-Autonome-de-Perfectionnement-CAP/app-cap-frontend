// ════════════════════════════════════════════════════════════════════════════
// Fichiers groupés — dashboards Direction avec circuit de correction
// Chaque section correspond à un fichier séparé à créer dans /dashboards/
// ════════════════════════════════════════════════════════════════════════════

// ════════════════════════════════════════════════════════════════════════════
// SecDirAdjointeDashboard.tsx
// ════════════════════════════════════════════════════════════════════════════
...
interface DirectionStats {
  total_in_progress: number; pending_at_my_level: number
  total_validated: number; total_rejected: number
}
      ...
      <DemandeDetailBase demande={demande}>
        {inCircuit ? (
          ...
          <CAlert color="info" className="mt-3 py-2 small">
            <strong>Information :</strong> En transmettant, le dossier passe à la Directrice Adjointe.
          </CAlert>
        )}
      </DemandeDetailBase>
    </DemandeModalShell>
...
export const SecDirAdjointeDashboard = () => {
  ...
  return (
    <DirectionShell title="Gestion des demandes" subtitle="Secrétariat — Directrice Adjointe" hideHeader>
      ...
      <CCard className="border-0" style={{ boxShadow: '0 2px 16px rgba(0,0,0,0.07)', borderRadius: 14 }}>
        <CCardHeader className="bg-white" style={{ borderBottom: '1px solid #f1f5f9', borderRadius: '14px 14px 0 0', padding: '16px 20px 12px' }}>
          <div className="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
              <div style={{ fontWeight: 700, fontSize: '1.05rem', color: '#111827' }}>Documents à transmettre</div>
              <div style={{ fontSize: '0.78rem', color: '#94a3b8', marginTop: 2 }}>{demandes.length} dossier{demandes.length > 1 ? 's' : ''} en attente</div>
            </div>
      ...
      {selected && <SecDaDetailModal demande={selected} visible={detailOpen} onClose={closeDetail} onAction={handleAction} />}
    </DirectionShell>
  )
}
...
// ════════════════════════════════════════════════════════════════════════════
// STOP — Ce fichier est groupé. Séparez chaque section dans son propre fichier.
// Les 3 dashboards suivants sont dans des fichiers séparés ci-dessous.
// ════════════════════════════════════════════════════════════════════════════
