...
const DetailModal = ({ demande, visible, onClose, onAction }: {
  ...
  // Dans SecDirAdjointeDashboard.tsx — remplacer uniquement le footer du DetailModal
  ...
  const footer = (<>
    ...
    <FlaggedValidationAction
      action="sec_da_transmit_flagged"
      loading={loading}
      run={run}
    ...
    <DemandeModalShell demande={demande} visible={visible} onClose={onClose}
      ...
      <DemandeDetailBase demande={demande}>
        <CAlert color="info" className="mt-3 py-2 small">
          <strong>Information :</strong> En transmettant, le dossier passe à la Directrice Adjointe pour signature.
        </CAlert>
      </DemandeDetailBase>
    </DemandeModalShell>
    <MotifModal
